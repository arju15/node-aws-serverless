...
module.exports.handler = async function (event, context, callback) {
    ...
    try{
        ...
        let fieldsObj = "user_master.user_id, user_master.username, user_master.first_name, user_master.last_name, user_master.profile_pic "
        ...
        let existingRecords = await DBManager.getJoinedData('follow_users', 'user_master', 'follow_user_id', 'user_id', fieldsObj, whereQry );
         if(existingRecords && existingRecords.length){
            response = { success: true, message: "Followings List", data: existingRecords };
        }
        else {
            response = { success: true, message: "No followings found."}
        }
