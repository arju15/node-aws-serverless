...
const updateStatus = function (id, status, parsedBody) {
    ...
    return new Promise((resolve, reject) => {
        ...
        QB.data.update(className, params, function (err, result) {
            if (result) {
                resolve(result);
            } else {
                // error
    ...
    try {
        ...
        if (ticket && ticket.length > 0 && '_id' in ticket[0] && 'event_status' in ticket[0] && 'event_id' in ticket[0] && 'tickets' in ticket[0]) {
           if (ticket[0].event_status === 'pending') {
                ...
                awsRequestHelper.callbackRespondWithSimpleMessage(callback, 400, 'event_status is with complete status');
            }
        } else {
            awsRequestHelper.callbackRespondWithSimpleMessage(callback, 400, 'Invalid TicketId');
        }
    } catch (err) {
        console.log("Error", err);
