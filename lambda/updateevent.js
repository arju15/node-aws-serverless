...
const addAdmins = async function(api_data,event,api_base_url){
    ...
    if(admins && admins.length > 0){
        ...
        for(let admin of admins){
            ...
            promises.push(AWSManager.sendEmail([admin.email],body,'You are now admin for a event!'));
        }
    ...
    if(guests && guests.length > 0){
        ...
        for(let guest of guests){
            ...
            if(newGuest){
                ...
                let mailSubject = `You are now invited to ${event.event_name}`;
...
const sendEventCreatedEmail = async function(event,api_data){
    ...
    let body = _.template(tmpl)(templateVars);
    await AWSManager.sendEmail([api_data.user_email],body,'You event is live!');
};
